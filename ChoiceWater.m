    ...
    if isempty(choice_acq.ChosenTarget) || choice_acq.ChosenTarget == 0
        ...
        if choice_acq.ChosenTarget == 2
            ...
            if ~hold1.Success
                ...
                chose_img1 = true;
                num_rew    = TrialRecord.User.img1_pulses;   % can be 0,1,2,3
            end
        ...
        elseif choice_acq.ChosenTarget == 4
            ...
            if ~hold2.Success
                ...
                chose_img2 = true;
                num_rew    = TrialRecord.User.img2_pulses;   % can be 0,1,2,3
            end
        end
    end
end
...
% ---------- Reward / error ----------
if error_type == 0
    % Only play "correct" sound when some reward is given, or always if you prefer.
    % Here: always play correct when a valid choice+hold was made.
...
bhv_variable('img1_index',    TrialRecord.User.img1_index);    % 1=Zero,2=w1,3=w2,4=w3
